...
def select_sensor_stream(data, select_sensor=0, sensor_axis=1):
    selected_data = np.take(data,indices=[select_sensor], axis=sensor_axis)
...
def main():
    ...
    for sensor_i in range(total_sensors):
        ...
        cbae_agent = agentNetwork.add_agent(name="CBAE_Agent_"+str(sensor_i+1), agentType=CBAE_Agent,
                                            conv_architecture=[1, 8, 3],
        ...
        cbae_agent.bind_output(aggregator, channel=["test","dmm_code"])

        # add to list of agents
        select_sensor_agents.append(select_sensor_agent)
        cbae_agents.append(cbae_agent)
    agentNetwork.bind_agents(minmax_agent, select_sensor_agents, channel="train")
    agentNetwork.bind_agents(set_drift_agent, select_sensor_agents, channel="test")
    ...
    aggregator.bind_output(sum_propagate_agent, channel=["test", "dmm_code"])
