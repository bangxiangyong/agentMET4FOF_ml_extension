...
from agentMET4FOF.agentMET4FOF.agents import AgentNetwork, MonitorAgent, AgentMET4FOF
...
class OrchestratorAgent(AgentMET4FOF):
    def init_parameters(self, bae_agent, bae_params={}):
    ...
    def on_received_message(self, message):
        ...
        if self.count <3 :
            self.send_set_attr(attr="perturbed_sensors", value=[1])  # send to set drift agent
            self.bae_params.update({"dense_architecture": [150*self.count]})
            self.bae_agent.init_parameters(**self.bae_params)

            # send to datastream agent to kickstart agent
...
def main():
    ...
    bae_params = {
        "conv_architecture": [total_sensors, 5, 3],
        ...
        "return_samples": True,
        "predict_train": False,
        # "posthoc_class": GradientShap  # {GradientShap, DeepLift}
    }
    ...
    mean_bae_agent = agentNetwork.add_agent(name="MeanBAESamples", agentType=ML_TransformAgent,
                                            model=mean_bae_samples, use_dmm=False)
    ...
    orchestrator_agent = agentNetwork.add_agent(agentType=OrchestratorAgent,
                                                bae_agent=cbae_agent,
                                                bae_params=bae_params)
    ...
    sma_agent.bind_output(mean_bae_agent, channel=["test", "dmm_code"])
    mean_bae_agent.bind_output(truncate_agent, channel=["test", "dmm_code"])

    # mean_bae_agent.bind_output(normalise_agent, channel=["test", "dmm_code"])
