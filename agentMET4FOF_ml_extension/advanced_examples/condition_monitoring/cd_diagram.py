...
for perf_key in ["gmean-sser","gmean-sdc","pearson","mcc"]:
    ...
    fig = draw_cd_diagram(df_perf=df_perf_cd, title=perf_key, labels=True)
    fig.tight_layout()
...
aggregate_perf_df(df_perf, perf_key = "pearson")
...
fig = draw_cd_diagram(df_perf=df_perf_cd_hieq, title=perf_key, labels=True)
fig.tight_layout()

# print table
aggregate_perf_df(hieq_df, perf_key = "hieq")

#========PERC. OF HIGH PEARSON CORR.=================
# df_perf = load_bae_results(filename=main_filename)
# perf_key = "pearson"
