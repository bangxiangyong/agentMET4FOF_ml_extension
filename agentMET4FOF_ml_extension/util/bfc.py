...
class FFT_BFC():
    ...
    def fit(self, x_data, y_data=None):
        ...
        for i in range(n_sensors):
            ...
            self.freq_of_sorted_values[i], __ = self.extractBFC_train(sensor=x_data[:, :, i], n_of_samples=n_of_samples,
                                                                      N=self.perc_feat)
        return self

    def fit_transform(self, x_data, y_data=None):
        """
        ...
        for i in range(n_sensors):
            ...
            # print("---------------------------------------------------------------------------------")
            # frequency and amplitudes
            self.freq_of_sorted_values[i], sorted_values_from_all_sensors[i] = self.extractBFC_train(
    ...
    def transform(self, x_data, y_data=None):
        ...
        for i in range(n_sensors):
            # print("Sensor number %s" % i)
            # print("---------------------------------------------------------------------------------")
            sorted_values_test[i] = self.extractBFC_test(sensor_test=x_data[:, :, i], n_of_samples=n_of_samples,
                                                         N=self.perc_feat, frequencies=self.get_best_frequencies()[i])
    ...
    def extractBFC_train(self, sensor, n_of_samples, N):
        ...
        sorted_values_matrix = fft_matrix.iloc[:, :int((N / 100.0) * len(freq))]
        ...
        sorted_values_matrix.columns = range(int((N / 100.0) * len(freq)))  # Resetting the column labels.
        # print("---------------------------------------------------------------------------------\n")
