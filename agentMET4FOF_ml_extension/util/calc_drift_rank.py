...
def calc_gmean_sser(sensor_nll,perturbed_sensors):
    ...
    return {"sensitivity":sensitivity,"specificity":specificity,"gmean-sser":gmean_sser}

def calc_sser_mcc(sensor_nll,perturbed_sensors):
    predicted_labels = predict_drift_labels(sensor_nll, k=len(perturbed_sensors))
    true_labels = get_true_labels(sensor_nll, perturbed_sensors)
    ...
    return {"mcc":matthew_corr,"gmean-sser":gmean_sser}

# def calc_pearson_corr():
