...
def plot_time_series_samples(x, y_target, metadata,  sensor_axis=1, figsize=(18,5), nrows=3, ncols=5):
    plots = []
    if isinstance(x, dict):
        for key in x.keys():
            plots.append(plot_time_series_samples_(x[key], y_target[key], metadata,
                                                   sensor_axis=sensor_axis,figsize=figsize,
                                                   nrows=nrows,ncols=ncols, suptitle="Axis: "+key))
    return plots

def plot_time_series_samples_(ts_x, y_target, metadata,  sensor_axis=1, figsize=(18,5), nrows=3, ncols=5, suptitle=""):
